...
namespace radar
{
    ...
    public:
        array(const T*, const radar::shape<N>&);
        array(cuda::shared_storage<T>*, const radar::shape<N>&, size_t, size_t);

        array(const radar::shape<N>&);
        template<typename InputIterator>
        array(const radar::shape<N>&, InputIterator, InputIterator);
        array(const radar::shape<N>&, const std::initializer_list<T>&);

        array(const array&);

        template<typename Expr>
        array(const radar::shape<N>&, const engine::expr<Expr>&);
        template<typename Expr>
        ...
        const radar::shape<N>& shape() const;
        size_t shape(size_t) const;

        template<size_t I>
        array<N - I, AD, T> at(const radar::shape<I>&) const;
        template <typename... Indices>
        array<N - sizeof...(Indices), AD, T> operator()(Indices...) const;

        template<size_t M>
        array<M, AD, T> reshape(const radar::shape<M>&) const;
    ...
    public:
        array(const T*, const radar::shape<N>&);
        array(cuda::shared_storage<T>*, const radar::shape<N>&, size_t);
        ...
        template<size_t I>
        array<N - I, false, T> at(const radar::shape<I>&) const;
        template <typename... Indices>
        array<N - sizeof...(Indices), false, T> operator()(Indices...) const;

        template<size_t M>
        array<M, false, T> reshape(const radar::shape<M>&) const;
        template<size_t I = N - 1>
        array<N - I, false, T> flatten() const;
    };

    template<bool AD = autodiff, typename T = real, size_t N>
    inline auto make_array(const radar::shape<N>&);

    template<bool AD = autodiff, typename InputIterator, size_t N>
    inline auto make_array(const radar::shape<N>&, InputIterator, InputIterator);
    template<bool AD = autodiff, size_t N, typename T>
    inline auto make_array(const radar::shape<N>&, const std::initializer_list<T>&);
...
{
    template<size_t N, bool AD, typename T>
    array<N, AD, T>::array(const T *device_ptr, const radar::shape<N> &shape)
    ...
    template<size_t N, typename T>
    array<N, false, T>::array(const T *device_ptr, const radar::shape<N> &shape)
        : cuda::shared_array<T>(device_ptr, shape.length()),
          _shape(shape)
    {}

    template<size_t N, bool AD, typename T>
    array<N, AD, T>::array(cuda::shared_storage<T> *storage, const radar::shape<N> &shape, size_t offset, size_t base_index)
    ...
    template<size_t N, typename T>
    array<N, false, T>::array(cuda::shared_storage<T> *storage, const radar::shape<N> &shape, size_t offset)
        : cuda::shared_array<T>(storage, shape.length(), offset),
          _shape(shape)
    {}

    template<size_t N, bool AD, typename T>
    array<N, AD, T>::array(const radar::shape<N> &shape)
    ...
    template<size_t N, typename T>
    array<N, false, T>::array(const radar::shape<N> &shape)
        : cuda::shared_array<T>(shape.length()),
          _shape(shape)
    {}
    ...
    template<size_t N, bool AD, typename T>
    template<typename InputIterator>
    array<N, AD, T>::array(const radar::shape<N> &shape, InputIterator first, InputIterator last)
        : cuda::shared_array<T>(shape.length()),
          _shape(shape),
          _grad_index(engine::get_tape<T>()->new_grad(shape.length()))
    {
    ...
    template<size_t N, typename T>
    template<typename InputIterator>
    array<N, false, T>::array(const radar::shape<N> &shape, InputIterator first, InputIterator last)
        : cuda::shared_array<T>(shape.length()),
          _shape(shape)
    {
        auto dist = std::distance(first, last);
    ...
    template<size_t N, bool AD, typename T>
    array<N, AD, T>::array(const radar::shape<N> &shape, const std::initializer_list<T> &data)
        : array(shape, data.begin(), data.end())
    {}

    template<size_t N, typename T>
    array<N, false, T>::array(const radar::shape<N> &shape, const std::initializer_list<T> &data)
        : array(shape, data.begin(), data.end())
    {}
    ...
    //todo: if expr is ad, compute radar of rvalue & push lvalue, do nothing otherwise
    template<size_t N, bool AD, typename T>
    template<typename Expr>
    array<N, AD, T>::array(const radar::shape<N> &shape, const engine::expr<Expr> &expr)
        : cuda::shared_array<T>(shape.length()),
          _shape(shape),
          _grad_index(engine::get_tape<T>()->new_grad(shape.length()))
    {
    ...
    template<size_t N, typename T>
    template<typename Expr>
    array<N, false, T>::array(const radar::shape<N> &shape, const engine::expr<Expr> &expr)
        : cuda::shared_array<T>(shape.length()),
          _shape(shape)
    {
        cuda::assign(this->data(), this->_size, engine::get_access(expr.self()));
    ...
    //todo: if expr is ad, compute radar of rvalue & push lvalue, probably clear radar data otherwise
    template<size_t N, bool AD, typename T>
    template<typename Expr>
    ...
    //todo: clear all previous gradient data, delete radar by index
    template<size_t N, bool AD, typename T>
    array<N, AD, T> &array<N, AD, T>::operator>>=(const array &other)
    {
        if (this == &other)
    ...
    template<size_t N, bool AD, typename T>
    const radar::shape<N> &array<N, AD, T>::shape() const
    {
        return _shape;
    }

    template<size_t N, typename T>
    const radar::shape<N> &array<N, false, T>::shape() const
    {
        return _shape;
    }
    ...
    template<size_t N, bool AD, typename T>
    template<size_t I>
    array<N - I, AD, T> array<N, AD, T>::at(const radar::shape<I> &index) const
    {
    ...
    template<size_t N, typename T>
    template<size_t I>
    array<N - I, false, T> array<N, false, T>::at(const radar::shape<I> &index) const
    {
        auto extents = _shape.template cut<I>();
        auto offset = _shape.offset(index);
    ...
    template<size_t N, bool AD, typename T>
    template<size_t M>
    array<M, AD, T> array<N, AD, T>::reshape(const radar::shape<M> &shape) const
    {
    ...
    template<size_t N, typename T>
    template<size_t M>
    array<M, false, T> array<N, false, T>::reshape(const radar::shape<M> &shape) const
    {
        if (this->_size != shape.length())
            throw std::invalid_argument("Array size mismatch.");
    ...
    inline auto make_array(const radar::shape<N>& shape, InputIterator first, InputIterator last)
    {
    ...
    inline auto make_array(const radar::shape<N>& shape, const std::initializer_list<T>& data)
    {
