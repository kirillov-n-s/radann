...
namespace radar::functor
{
    struct dot
    {
        ...
        {
            auto res = make_array<ADLhs || ADRhs, T>(radar::make_shape());
    ...
    struct outer
    {
        ...
        auto operator()(const array<1, ADLhs, T> &x, const array<1, ADRhs, T> &y) const
        {
            auto rows = x.size();
            auto cols = y.size();
            auto res = make_array<ADLhs || ADRhs, T>(radar::make_shape(rows, cols));
    ...
    struct trans
    {
        ...
        auto operator()(const array<2, AD, T> &x) const
        {
            auto rows = x.shape(0);
            auto cols = x.shape(1);
            auto res = make_array<AD, T>(radar::make_shape(cols, rows));
